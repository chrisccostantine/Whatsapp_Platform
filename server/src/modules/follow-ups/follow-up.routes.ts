...
followUpRouter.get("/", asyncHandler(async (req, res) => {
  ...
  const dateWhere: Prisma.FollowUpWhereInput = query.view === "today" ? { dueAt: { gte: today, lt: tomorrow } } : query.view === "upcoming" ? { dueAt: { gte: tomorrow } } : query.view === "overdue" ? { dueAt: { lt: now }, status: { in: ["PENDING", "IN_PROGRESS"] } } : {};
...
followUpRouter.patch("/:id/status", requireRole("OWNER", "ADMIN", "SALES_AGENT"), asyncHandler(async (req, res) => {
  ...
  const existing = await prisma.followUp.findFirstOrThrow({ where: { id: routeParam(req.params.id), businessId: req.auth!.businessId, deletedAt: null, ...(req.auth!.role === "SALES_AGENT" ? { assignedUserId: req.auth!.userId } : {}) } });
