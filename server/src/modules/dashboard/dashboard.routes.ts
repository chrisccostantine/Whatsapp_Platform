...
dashboardRouter.get("/summary", asyncHandler(async (req, res) => {
  ...
  const [customers, newLeads, followUpsToday, openLeads, wonLeads, recentActivity, stages] = await prisma.$transaction([
  ...
  return ok(res, { metrics: { customers, newLeads, followUpsToday, openLeads, conversionRate: totalClosed ? Math.round((wonLeads / totalClosed) * 1000) / 10 : 0 }, leadsByStage: stages.map((s) => ({ name: s.name, value: s._count.leads, color: s.color })), recentActivity });
}));
