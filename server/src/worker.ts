...
import { generateOperationalNotifications } from "./modules/notifications/notification.service.js";
const webhookWorker = startWhatsAppWorker();
const campaignWorker = startCampaignWorker();
void generateOperationalNotifications().catch((error) => console.error("Initial notification sweep failed", error));
const notificationTimer = setInterval(() => { void generateOperationalNotifications().catch((error) => console.error("Notification sweep failed", error)); }, 60_000);
...
const shutdown = async () => { clearInterval(notificationTimer); await Promise.all([webhookWorker.close(), campaignWorker.close()]); await prisma.$disconnect(); process.exit(0); };
